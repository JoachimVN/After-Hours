...
public final class NoGameView {
   ...
   */
  public record Message(String text, double displaySecs, double gapSecs, boolean appendBelow,
                        boolean holdOverlay, boolean fadeIn) {
    /**
  ...
  public static Parent build(List<Message> monologue, String contextMessage,
    ...
    for (int i = 0; i < monologue.size(); i++) {
      if (monologue.get(i).text().isEmpty()) {
        ctxIdx = i;
        break;
      }
    }
    ...
    for (int i = 0; i < messages.size(); i++) {
      ...
      if (entry.gapSecs() > 0) {
        ...
        if (entry.holdOverlay() && !currentLabels.isEmpty()) {
          ...
          FadeTransition fadeOutLabel =
              new FadeTransition(Duration.millis(isLast ? 3600 : 1200), labelRef);
  ...
  private static void playSequenceWithButtonHooks(
    ...
    for (int i = 0; i < msgs.size(); i++) {
      ...
      if (t.equals("Here let me bring it closer")) {
        closerIdx = i;
      }
      if (t.equals("Let me bring the button back")) {
        bringBackIdx = i;
      }
      if (t.equals("Thank you, thank you.")) {
        thanksIdx = i;
      }
    }
    ...
    if (closerIdx >= 0) {
      hooks.put(closerIdx, moveButtonToCenter);
    }
    if (bringBackIdx >= 0) {
      hooks.put(bringBackIdx, moveButtonBack);
    }
    ...
    if (onComplete != null) {
      seqB.setOnFinished(e -> onComplete.run());
    }
