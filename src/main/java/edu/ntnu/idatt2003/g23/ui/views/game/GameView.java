...
public final class GameView {
    ...
    private void rebuildDetail() {
        ...
        incBtn.setOnAction(e -> {
            amountTracksSell[0] = false;
            try {
                quantityField.setText(String.valueOf(NumberParser.parse(quantityField.getText()).intValue() + 1));
            } catch (NumberFormatException ignored) {
                quantityField.setText("1");
            }
            normalizequantityAndAmount.run();
        ...
        final Runnable updateBuyAmount = () -> {
            int quantity;
            try { quantity = NumberParser.parse(quantityField.getText()).intValue(); }
